...
def validate_data(df):
    ...
    if not rapport_df.empty:
        rapport_df.to_csv("rapport_validation.csv", index=False)
        raise ValueError(f"Lot rejeté — {len(rapport_df)} violations détectées. Voir rapport_validation.csv")
    return rapport_df
...
def main():
    ...
    df = fix_amounts(df)
    # validation avant toute transformation
    validate_data(df)
